...
export async function getHomepageStats() {
  try {
    ...
    // Calculate price points checked (11 providers from cron routes * average GPUs per provider)
    // We monitor 11 providers via cron jobs, and each provider typically has 5-15 GPU models
    const cronProviders = 11; // From app/api/cron/all/route.ts CRON_ROUTES array
    const avgGpusPerProvider = Math.ceil(gpuData.length / providerData.length);
    const totalPricePoints = cronProviders * avgGpusPerProvider;

    return {
      gpuCount: gpuData.length,
      providerCount: providerData.length,
      pricePointsChecked: totalPricePoints
    };
  } catch (error) {
