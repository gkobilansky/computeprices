...
export async function getProviderBySlug(slug) {
  // Get database providers to merge type info
  const dbProviders = await fetchProviders();

  // First try to find in JSON data
  const jsonProvider = providersData.find(p => p.slug === slug);
  if (jsonProvider) {
    // Merge with DB data to get type field
    const dbProvider = dbProviders.find(p =>
      p.id === jsonProvider.id ||
      p.slug === slug ||
      p.name.toLowerCase().replace(/\s+/g, '-') === slug
    );
    return {
      ...jsonProvider,
      id: dbProvider?.id || jsonProvider.id,
      type: dbProvider?.type || 'gpu_compute'
    };
  }
