...
import { listGPUs } from '@/lib/hooks/useGPUData';
...
export default function GPUPage() {
  ...
  useEffect(() => {
    const gpuData = gpuData.find(p => p.slug === params.slug);
    setGPU(gpuData || null);
    setLoading(false);
  }, [params.slug]);
  ...
  if (!gpu) {
    return <div className="min-h-screen flex items-center justify-center">
      <div className="text-center">
        <h2 className="text-2xl font-bold mb-4">GPU not found</h2>
        <Link href="/gpus" className="btn btn-primary">
          View All GPUs
        </Link>
      </div>
    </div>;
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8 space-y-12">
      <nav className="mb-8">
        <Link href="/gpus" className="text-primary hover:underline mb-4 block">
          ← Back to GPUs
        </Link>
      </nav>
